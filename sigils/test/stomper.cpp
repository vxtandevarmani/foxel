...
typedef VOID ( *ScEntry )(void*, void*);
...
int main(
    ...
    struct packet relocs = {0};

    struct param args = {0};

    args.length = 17;
    args.name = (char*)HeapAlloc(GetProcessHeap(), 
                                0x8, args.length);
    ...
    relocs.base   = (uintptr_t)entry,
    relocs.length = (uintptr_t)file_length,
    printf( "[*] target code section @ %llx [%ld bytes]\n", image_base, sec_header->SizeOfRawData );
    printf( "[*] entry point @ %p \n", entry );

    puts( "[*] lets press this shit yo" );
    printf( "[*] press enter..." );
    getchar();


    entry( (void*)&relocs, (void*)&args ); // nullptr
...
SECTIONS
{
    .text :
    {
        *(.text$B)
        __my_boundry = .;
        *(.rdata*)   .rdata for windows .rodata for riscv
        LONG(__my_boundry);
    }
}
...
CFLAGS += -fno-exceptions -fms-extensions -fPIC -Iinclude
CFLAGS += -Wl,-s,-Tscripts/linker.ld,--no-seh,--enable-stdcall-fixup
